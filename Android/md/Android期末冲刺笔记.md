...
**2022-2023 真题分析**：
- **Handler 线程通信** → 网络请求题（35分）必考，用于子线程向主线程发消息
- **ListView 点击事件** → 列表题常考，点击 item 跳转并传参
- **广播动态注册** → 单独出题（10分），注册 + 发送 + 注销三件套
- **JSON 嵌套解析** → `result` → `list` → 对象，层级不能搞错

**新增到必背清单**：
- Handler 定义（主线程）+ Message.obtain + sendMessage
- ListView.setOnItemClickListener + position 取数据
- 广播：IntentFilter + registerReceiver + unregisterReceiver
...
### 12. Handler 线程通信（必背 ★★★★）
**可能出题形式**：网络请求 + Handler 更新 UI

**为什么需要 Handler？**
- Android 规定：只能在主线程更新 UI
- 网络请求必须在子线程执行
- Handler 用于从子线程向主线程发消息
...
**记忆口诀**：主线程定义 Handler → 子线程 obtain 消息 → sendMessage 发送

### 13. ListView 点击事件（必背 ★★★）
**可能出题形式**：点击 item 跳转并传参
...
listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        // 取出点击的数据
        Student student = students.get(position);  // 用 position 取数据
...
**记忆技巧**：
- 四个参数记住 `position`（其他三个基本不用）
- 用 `position` 从数据源 `list.get(position)` 取对象
- 跳转 + 传参 用 Intent

### 14. HttpURLConnection POST 请求（必背 ★★★）
```java
