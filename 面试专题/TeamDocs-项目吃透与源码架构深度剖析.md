...
- [九、 面试高频题库全景通关清单（8组核心考点 + 必考场景题）](#九-面试高频题库全景通关清单8组核心考点--必考场景题)
  ...
  - [9.9 第九组：必考高频实战场景题](### 9.10 第十组：链路问题)
...
```
第一，Redis 的常规命令主要在内存中处理，访问路径较短。MySQL 也有 InnoDB Buffer Pool，热点数据命中时不需要每次读磁盘，所以不能把两者的差异简单归结为“Redis 读内存、MySQL 每次读磁盘”。
...
第二层是区分可降级功能和安全校验。Redis 除了最近浏览和限流，还保存 JWT 撤销记录与用户改密时间戳水位；辅助功能可以评估降级，但不能在无法检查撤销状态时直接放行受保护请求。

比如最近浏览记录属于辅助功能，可以设计为 Redis 不可用时跳过记录或返回空列表；但打开、编辑文档能否继续，还取决于认证和权限校验，不能由最近浏览可降级推导出整个请求一定可用。

限流这块就需要根据具体接口的重要程度处理。Redis 不可用时，可以采用本机限流作为临时降级方案，或者对核心接口采用更保守的保护策略，避免因为 Redis 故障导致整个服务不可用。

另外，文档、空间等核心业务数据保存在 MySQL 中，不会仅因 Redis 故障就从 MySQL 丢失。但数据仍在不等于服务仍可用；Redis 恢复时也要考虑撤销记录、改密水位等安全状态是否完整，不能只恢复缓存和最近浏览。
```
...
- [ ] **Q50：什么是聚簇索引？**
> 如果一张 InnoDB 表没有定义主键，InnoDB 会选择一个合适的唯一非空索引作为聚簇索引；如果也没有这样的索引，就会自动生成隐藏的行 ID 作为聚簇索引
```
聚簇索引可以理解为，索引的叶子节点直接保存了这一行完整的数据记录。

在 MySQL InnoDB 中，主键索引就是聚簇索引。也就是说，InnoDB 会按照主键的顺序组织数据，主键索引的叶子节点中保存的就是完整的行数据。

所以通过主键查询时，找到主键索引的叶子节点之后，就可以直接拿到完整的数据，不需要再通过其他索引查一次。

例如 TeamDocs 中一张文档表，如果 document_id 是主键，那么主键索引就是聚簇索引。按照 document_id 找到对应的叶子节点后，就可以直接得到这条文档记录的完整字段
```
...
二级索引的叶子节点主要是索引字段和主键
```
二级索引，也叫辅助索引，是除了聚簇索引之外建立的索引。

它的叶子节点不会保存完整的行数据，而是保存索引字段的值和对应的主键值。

例如一张文档表的主键是 document_id，现在给 title 建立一个普通索引。

这个 title 索引就是二级索引。通过 title 查询时，先在 title 的 B+ 树中找到对应记录，叶子节点拿到的是对应的 document_id，然后再根据 document_id 去聚簇索引中找到完整的行数据。
...
- [ ] **Q52：什么情况下会回表？**
```
回表是指使用二级索引查询时，二级索引的叶子节点中只有索引字段和主键值，而查询还需要其他字段，所以需要再根据主键去聚簇索引中查询完整的行数据，这个过程就叫回表

判断是否回表：直接看二级索引能不能直接提供查询需要的所有字段
```

---

- [ ] **Q53：联合索引为什么有最左匹配原则？**
>联合索引按照字段从左到右进行字典序排序，通常利用连续的最左前缀定位记录，这就是**最左匹配原则**；不满足前缀不等于索引在所有执行方式下都不可用。
```
联合索引就是把多个字段按照一定顺序建立在同一个索引中，比如建立一个联合索引：(a, b, c)

它在 B+ 树中并不是分别建立三个独立索引，而是按照 a、b、c 的顺序组织索引数据。
可以把它理解成先按照 a 排序，在 a 相同的情况下再按照 b 排序，在 a、b 都相同的情况下再按照 c 排序。
所以通常要利用最左前缀才能有效缩小索引查找范围；覆盖索引扫描、满足条件时的索引跳跃扫描等是另外的执行方式，后续字段也可能用于索引条件下推，具体应看 EXPLAIN。
...
- [ ] **Q54：什么情况下索引会失效？**
>以下情况可能影响普通 B+ 树索引的范围定位，或使优化器选择全表扫描，并非一律“索引完全失效”。是否使用索引要结合表达式索引、转换方向、数据分布与 EXPLAIN 判断；成本上不选择索引也不等于索引失效。
```
第一，在索引列上进行函数运算或者表达式计算
...
第三，使用前导模糊查询
	WHERE name LIKE '%abc'
	WHERE name LIKE '%abc%'
	因为开头是通配符，MySQL 无法根据 name 的最左边开始确定索引范围，所以普通 B+ 树索引通常无法有效使用
第四，违反联合索引的最左匹配原则
第五，索引列参与隐式类型转换
第六，查询的数据量太大
```
...
>使用 MySQL 的 EXPLAIN 查看执行计划
>判断 SQL 有没有使用索引，首先看 EXPLAIN 的 key；key 是 NULL 通常表示没有使用索引，key 有值说明使用了对应索引，然后再结合 type、rows 和 Extra 判断执行效率
```
例如：EXPLAIN SELECT * FROM document WHERE title = '项目文档';
...
| --------------- | -------- | ------------- |
| `type`          | 怎么访问数据   | **访问方式**      |
| `possible_keys` | 可能用哪些索引  | **候选索引**      |
| `key`           | 实际用了哪个索引 | **最终索引**      |
| `Extra`         | 额外执行信息   | **有没有回表、排序等** |
```
type: ref 查询方式
possible_keys: idx_title 候选索引
key: idx_title 最终索引
Extra: Using index 直接从索引中获取了数据，没有回表
```
...
A 原子性保证事务要么全部成功，要么全部失败。
	例如用户 A 给用户 B 转账 100 元，需要先扣除 A 的 100 元，再增加 B 的 100 元。如果 A 扣款成功，但是 B 加钱失败，那么整个事务应该回滚，A 的 100 元也要恢复
	
C 一致性保证事务执行前后数据符合规则。
	例如转账前 A 和 B 的账户余额总和是 1000 元，那么正常转账完成后，两个人的余额总和仍然应该是 1000 元
	
I 隔离性保证多个事务并发执行时互相不会产生不合理的影响。

	例如事务 A 正在修改账户余额，但是还没有提交，这时候事务 B 不应该直接看到事务 A 修改但尚未提交的数据
	
D 持久性保证事务提交之后，数据不会因为后续故障而轻易丢失
	在 InnoDB 中，主要通过 redo log 等机制保证已经提交的事务能够在故障恢复后重新应用
```
...
|读未提交|可能|可能|可能|高|
|读已提交|不会|可能|可能|较高|
|可重复读|不会|不会|InnoDB 有专门机制处理|较高|
|串行化|不会|不会|不会|最低|
```
第一，读未提交，Read Uncommitted
	一个事务可以读取到另一个事务还没有提交的数据，所以可能出现脏读
	
第二，读已提交， Read Committed
	一个事务只能读取其他事务已经提交的数据，可以解决脏读问题。但是同一个事务中，两次执行相同的查询，可能得到不同的结果，因为两次查询之间其他事务可能提交了新的数据，这叫不可重复读

第三，可重复读， Repeatable Read
	在同一个事务中，多次读取同一条数据，读取结果保持一致，可以解决不可重复读问题。
MySQL InnoDB 默认使用的就是可重复读

第四，串行化，Serializable
	它会让事务之间按照一定顺序执行，并发程度最低，但是能够提供最高级别的隔离性，可以避免脏读、不可重复读和幻读
```

---

- [ ] **Q59：为什么 MySQL 默认使用 RR（可重复读）？**
> MySQL 默认使用 RR，是因为它通过 MVCC 和锁机制，在保证较强事务隔离性的同时，又能够保持较好的并发性能，是比较适合通用业务场景的默认选择
```
第一，RR 能保证同一个事务中多次读取同一份数据时结果保持一致，可以避免不可重复读。

第二，InnoDB 通过 MVCC，也就是多版本并发控制，让普通查询可以读取事务对应的数据版本，不需要因为普通查询而大量加锁，因此能够保持比较好的并发性能。

第三，InnoDB 在 RR 下还结合 Next-Key Lock，也就是记录锁和间隙锁，对当前读进行控制，可以减少并发事务产生幻读的问题。

第四，如果默认使用串行化，虽然隔离性更强，但是大量事务需要等待，数据库并发性能会明显下降
```

---

- [ ] **Q60：什么是 MVCC（多版本并发控制）？**
> 数据版本、Read View、Undo Log
```
MVCC 就是多版本并发控制。

它的核心思想是给数据保留多个历史版本，让不同事务可以读取自己应该看到的数据版本，从而减少读写之间的锁冲突，提高并发性能。

InnoDB 主要通过 Undo Log 保存历史版本，通过 Read View 判断当前事务能够看到哪个版本。

简单来说就是：

**Undo Log 保存旧版本，Read View 判断版本是否可见。**

MVCC 主要用于普通的快照读，像 SELECT ... FOR UPDATE 这种当前读，主要依靠锁机制
```

---

- [ ] **Q61：什么是当前读和快照读？**
- 快照读就是读取事务开始时或者当前事务对应 Read View 中可见的数据版本，不需要加锁，普通的 SELECT 就属于快照读，主要依靠 MVCC。
- 当前读就是读取数据库中最新的数据版本，并且通常需要加锁
```
普通 SELECT 是快照读
SELECT ... FOR UPDATE 和 SELECT ... LOCK IN SHARE MODE 属于当前读
```
...
- 优化 = 统一加锁顺序、缩短事务、缩小锁范围
```
事务 A 先锁住数据 1，再等待数据 2。
事务 B 先锁住数据 2，再等待数据 1。
两个事务互相等待，就形成死锁
```
- 排查时，我一般先查看 MySQL 的死锁日志，可以执行：`SHOW ENGINE INNODB STATUS;`重点看 `LATEST DETECTED DEADLOCK`，可以看到发生死锁的事务、持有哪些锁、正在等待哪些锁，从而找到具体的 SQL
- 解决死锁通常从几个方面入手：让多个事务按照相同顺序获取锁、缩短事务执行时间、减少锁的范围，以及给查询条件建立合适的索引，减少锁住的数据范围

---

- [ ] **Q63：如果项目中的文档查询突然变慢，你怎么系统排查？**
```
我会按照从应用到数据库 -> 基础设施的顺序排查，而不是直接修改 SQL。

第一，先确认问题范围。看是所有文档查询都变慢，还是某几个接口、某类数据变慢，同时看接口耗时和错误日志。

第二，检查 Redis。如果这个查询使用了 Redis，就看 Redis 是否连接异常、响应变慢或者命中率下降。

第三，检查 MySQL。先用慢查询日志定位具体 SQL，再用 EXPLAIN 看执行计划，重点检查有没有走索引、type、key、rows 和 Extra。还要检查数据库连接池、CPU、内存和锁等待情况。

第四，检查 SQL 和索引。如果发现索引失效、全表扫描或者回表数据量过大，就优化 SQL 或者调整索引。

第五，再检查服务器资源，比如 CPU、内存、磁盘 I/O 和网络，确认是不是基础设施出现瓶颈。

最后根据排查结果处理，并通过压测或监控确认优化确实生效。

所以我的排查思路是：

先定位接口 → 再看 Redis → 再看 MySQL 和 SQL 执行计划 → 再检查索引和锁 → 最后检查服务器资源
```
...
```
修改时带上版本号，只有版本号没变才能更新，更新成功后版本号加一，版本号不一致就说明发生了并发修改
```
...
> 使用乐观锁或者幂等处理
```
两个用户同时删除时，让删除操作具备幂等性，第一次删除成功，后面的重复删除识别为已经删除，不重复处理
```
...
```
以下是可选设计方案，不代表 TeamDocs 当前已经实现 Redis 分布式上传锁。
1.幂等覆盖 / 秒传
	一种方案是 Redis 分布式锁 + 受权限约束的秒传
	第一个请求到达，拿文件 MD5/SHA 去 Redis 请求分布式锁：
		SET lock:upload:{MD5} client_id NX PX 30000。
		30 秒只是示例租期，大文件上传可能超时，需要续期或其他互斥设计；释放时必须原子校验 client_id。内容哈希不能代替资源访问权限校验。
	请求 A 获取锁成功：查询数据库/OSS，发现文件未存在，开始流式传输并写入 OSS；写入成功后更新数据库记录
	请求 B 获取锁失败：说明正在上传中，此时采用以下两种策略之一：
	(1). 轮询等待：请求 B 挂起并轮询 Redis 中的上传状态，等请求 A 完成后直接返回同一个文件 URL（即“秒传”）。
	(2). 直接提示：返回 409 Conflict 或提示文件正在上传中
	
2.秒级并发切片上传
	多段上传允许在同一个 UploadId 下使用相同 PartNumber 重新上传并替换分片，合并时仍需使用正确的分片清单和 ETag。它不保证不同 UploadId 之间去重，也不代表整个上传、合并和数据库落库流程天然幂等。
```
...
> Redis 保证操作原子性，主要是为了避免多个请求并发执行时操作互相穿插，导致数据状态不正确

---

- [ ] **Q68：Lua 脚本为什么可以保证多个 Redis 操作一起执行？**
> Redis Lua 脚本执行期间不会被其他命令穿插，适合原子完成条件判断与修改。但运行时出错不会自动回滚脚本此前已经完成的写入，也不能把 MySQL 等外部操作纳入同一原子事务。

---

- [ ] **Q69：Java 中 synchronized 是怎么实现的（锁升级过程）？**
```
在早期 Java 中，可以简单理解为 synchronized 会通过对象头中的锁状态和 Monitor 来管理线程竞争。
锁升级一般可以理解为： 无锁 → 偏向锁 → 轻量级锁 → 重量级锁。
无锁：没有线程竞争。
偏向锁：只有一个线程反复进入同步代码块，锁会偏向这个线程，减少加锁开销。
轻量级锁：出现少量线程竞争时，通过 CAS 等方式尝试获取锁，避免直接进入重量级锁。
重量级锁：竞争比较激烈时，会使用 Monitor，让没有拿到锁的线程阻塞等待。
所以锁升级的核心思想就是：
	竞争越激烈，锁的实现方式越重，从而在低竞争时减少开销，高竞争时保证线程安全。

不过需要注意，**从 JDK 15 开始偏向锁已经被默认禁用，并在 JDK 18 移除**，所以现在面试时最好不要把偏向锁说成现代 JDK 一定存在的实际阶段。讲锁升级时可以说明这是经典的 synchronized 锁优化过程。
```

---

- [ ] **Q70：volatile 有什么作用（可见性与有序性/内存屏障）？**
> 保证了**可见性和有序性** 但是不能保证复合操作的原子性

- **可见性：** 一个线程修改了 volatile 变量后，其他线程能够及时看到最新值，不会一直使用自己的工作内存中的旧值。
- **有序性：** volatile 会通过内存屏障限制编译器和 CPU 对相关指令进行重排序，保证特定的执行顺序

---

- [ ] **Q71：CAS 是什么？底层原理和 ABA 问题怎么解决？**
> **CAS 是一种乐观锁机制**，它包含三个操作数：内存位置（V）、预期值（A）和新值（B）
- 它会比较内存中的当前值和预期值，如果两者相等，就把它修改成新值；如果不相等，就说明有其他线程修改过，当前操作失败

---

- [ ] **Q72：ConcurrentHashMap 为什么线程安全？**
> 读基本无锁，写入用 CAS，冲突时对桶加 synchronized，不锁整个 Map

- 使用**细粒度的锁和 CAS** 保证线程安全
- 在 JDK 8 中，它的底层主要是数组加链表或者红黑树
- **读取**操作通常不需要加锁，可以**直接读取**，所以并发性能比较高
- **写入**时，如果对应位置没有数据，会通过 CAS 尝试插入；如果发生冲突，则会对对应的桶加 synchronized 锁，而不是锁住整个 Map
这样不同位置的数据可以同时进行操作，减少锁竞争
所以 ConcurrentHashMap 相比直接给整个 HashMap 加一把大锁，并发性能更好

| 对比                | HashMap    | Hashtable          | ConcurrentHashMap        |
| ----------------- | ---------- | ------------------ | ------------------------ |
| 线程安全              | ❌          | ✅                  | ✅                        |
| 数据结构              | 数组 链表 红黑树  | 数组 链表              | 数组 链表 红黑树                |
| 并发性能              | 高，但不适合并发修改 | 较低                 | 高                        |
| 加锁方式              | 不加锁        | 方法级 `synchronized` | **CAS + `synchronized`** |
| 是否允许 `null` key   | 允许 1 个     | 不允许                | 不允许                      |
| 是否允许 `null` value | 允许         | 不允许                | 不允许                      |
| 推荐使用              | 单线程场景      | 基本不推荐              | **并发场景**                 |

---

- [ ] **Q73：线程池的 7 个核心参数是什么？拒绝策略有哪些？**

- **`corePoolSize`（核心线程数）**：线程池中常驻的最小线程数量。即使线程空闲，也不会被销毁（除非设置了 `allowCoreThreadTimeOut`）。
    
- **`maximumPoolSize`（最大线程数）**：线程池允许创建的最大线程数量。当阻塞队列满了之后，会继续创建新线程，直到达到该上限。
    
- **`keepAliveTime`（非核心线程存活时间）**：当线程数超过 `corePoolSize` 时，多余的空闲线程在终止前等待新任务的最长时间。
    
- **`unit`（时间单位）**：`keepAliveTime` 的时间单位（如 `TimeUnit.SECONDS`）。
    
- **`workQueue`（任务阻塞队列）**：用于保存等待执行任务的阻塞队列（如 `ArrayBlockingQueue`、`LinkedBlockingQueue`、`SynchronousQueue` 等）。
    
- **`threadFactory`（线程工厂）**：用于创建新线程的工厂，通常用于定制线程名称、优先级或设置为守护线程（推荐使用 Guava 或自定义工厂赋予有意义的线程名，方便排查问题）。
    
- **`handler`（拒绝策略）**：当线程池和阻塞队列都满了，无法再接收新任务时执行的应对策略
```Java
...
提交任务 -> 核心线程未满？
            ...
            └─ 已满 -> 阻塞队列未满？
                        ...
                        └─ 已满 -> 线程数 < 最大线程数？
                                    ├─ 是 -> 创建非核心线程执行任务
                                    └─ 否 -> 触发拒绝策略(RejectedExecutionHandler)
```
常见的 4 种拒绝策略是：

- AbortPolicy：直接抛出异常，也是默认策略。
- CallerRunsPolicy：线程池未关闭时，让提交任务的线程执行；线程池已关闭时会丢弃该任务。
- DiscardPolicy：直接丢弃新任务，不抛异常。
- DiscardOldestPolicy：丢弃队列中最旧的任务，然后尝试提交当前任务

| **拒绝策略类**                         | **行为表现**                                       | **适用场景**                                            |
| --------------------------------- | ---------------------------------------------- | --------------------------------------------------- |
| **`AbortPolicy`（默认策略）**           | 直接抛出 `RejectedExecutionException` 异常，中断当前任务提交。 | **默认首选**。能够及时发现系统过载并报警。                             |
| **`CallerRunsPolicy`（调用者运行）**     | 线程池未关闭时，由**提交任务的线程同步执行**；已关闭时丢弃任务。 | **反压 / 降低提交速度**。不提供持久化、强一致性或绝不丢任务的保证。 |
| **`DiscardPolicy`（丢弃策略）**         | **静默丢弃**当前无法处理的任务，不抛异常也不做任何通知。                 | **允许丢包的边缘业务**（如日志收集、监控指标打点）。                        |
| **`DiscardOldestPolicy`（丢弃最老策略）** | 丢弃阻塞队列中**最早未处理的任务（队头任务）**，然后重新尝试提交当前任务。        | **注重时效性**的业务（更关心最新的数据/任务）。                          |
...
### 9.8 第八组：[Spring 框架底层](obsidian://open?vault=Java-notes&file=Javaweb%2FSpringAOP)
- [ ] **Q74：Spring IOC 是什么？控制反转反转了什么？**
> Spring IOC 就是控制反转，也就是把对象的创建和依赖关系管理交给 Spring 容器，而不是由程序自己 `new` 对象 
> 反转的是**对象创建和依赖关系的控制权**
...
> 是面向切面编程，能够将那些与业务无关,却为业务模块所共同调用的逻辑封装起来，以减少系统的重复代码，降低模块间的耦合度

> 在 TeamDocs 项目中，AOP 主要用于统一处理一些公共的横切逻辑，比如**权限校验、操作日志**

---

- [ ] **Q76：AOP 的底层原理是什么（JDK 动态代理 vs CGLIB）？**
**第一种是 JDK 动态代理。**
- 要求目标类实现接口。Spring 根据接口创建代理对象，通过代理对象调用方法。

**第二种是 CGLIB。**
- 不要求目标类实现接口，它通过继承目标类生成代理对象，然后重写方法，在方法前后加入额外逻辑

---

- [ ] **Q77：Spring Bean 的生命周期是什么？**
```Java
[1. 扫描与加载] ──> [2. 实例化] ──> [3. 属性注入] ──> [4. Aware 接口回调]
                                                             │
[7. 准备就绪/使用] <── [6. 初始化] <── [5. BeanPostProcessor 前置处理]
       │
[8. 容器关闭/销毁]
```
第一步，**实例化**，Spring 创建 Bean 对象。
第二步，**属性注入**，给 Bean 注入它依赖的其他对象，比如 `@Autowired`。
第三步，**初始化**，执行初始化方法，比如 `@PostConstruct`。
第四步，**使用**，Bean 创建完成后，就可以被业务代码使用。
第五步，**销毁**，Spring 容器关闭时执行销毁方法，比如 `@PreDestroy`


---

- [ ] **Q78：`@Transactional` 是怎么实现的？**
> `@Transactional` 的底层原理主要是 **Spring AOP + 动态代理**
> 调用代理对象 → 开启事务 → 执行业务代码 → 成功提交，异常回滚
```
当一个方法加上 `@Transactional` 后，Spring 会为这个 Bean 创建代理对象。
调用这个方法时，不是直接执行原方法，而是先经过代理。代理会先开启事务，然后执行真正的业务代码。
如果业务执行成功，就提交事务；如果发生符合条件的异常，就回滚事务
```
```Java
[ @Transactional 注解 ] 
          │ (1. 解析)
          ▼
[ TransactionInterceptor ] ──(2. 拦截请求)──> [ TransactionManager ]
                                                      │
                                                      ├── 3. 开启事务 (setAutoCommit(false))
                                                      ├── 4. 执行业务逻辑 (joinPoint.proceed())
...
> 异常有没有抛出来、异常类型、有没有经过 Spring 代理、Bean 是否由 Spring 管理，以及事务配置是否正确
```
第一，异常被 try-catch 吞掉。如果异常被捕获后没有继续抛出，Spring 感知不到异常，事务可能正常提交。

第二，抛出受检异常。Spring 默认只对 RuntimeException 和 Error 回滚，受检异常默认不会回滚，可以使用 rollbackFor = Exception.class。

第三，同类内部调用。如果一个方法通过 this.xxx() 调用另一个 @Transactional 方法，会绕过 Spring 代理，所以事务可能不生效。

第四，对象不是 Spring 管理的 Bean。如果自己 new 出来的对象没有经过 Spring 代理，事务不会生效。

第五，事务配置不正确。比如多数据源情况下事务管理器配置错误，也可能导致事务没有作用到正确的数据源。

第六，方法访问权限不合适。比如把 @Transactional 放在 private 方法上，Spring AOP 通常无法正常代理。
```
...
- [ ] **Q80：你项目里面哪些地方使用了事务？**
```
4 处默认传播行为（REQUIRED）：集中在多表原子写入和级联批量清理
1 处特殊传播行为（REQUIRES_NEW）：用于系统审计日志的独立持久化

场景 1：创建空间 —— 多表原子创建
场景 2：删除文件夹 —— 级联批量逻辑删除
场景 3 & 4：标签解绑与文档硬删除 —— 关联表原子清理

审计操作日志落库——REQUIRES_NEW 
```

---

- [ ] **Q81：如果 MinIO 成功，但是数据库失败，`@Transactional` 能不能回滚 MinIO？**
> `@Transactional` **只能回滚数据库，不能回滚 MinIO；如果 MinIO 成功、数据库失败，需要额外的删除或补偿机制处理**
---
...
- [ ] **Q99：【重点】删除/访问 Document 链路：一个请求到底怎么完成“认证 → 授权 → 资源校验 → 数据操作”？**
...
6. 为什么使用 JWT？
7. JWT 的基本结构是什么？
8. Spring Security 在哪里发挥作用？
9. JWT Filter 做什么？
10. JWT 无状态，为什么还需要 Redis？
11. 用户退出以后 Token 怎么失效？
12. 修改密码以后旧 Token 怎么失效？
13. BCrypt 做什么？

### 权限

14. 为什么有 OWNER / ADMIN / MEMBER？
15. 为什么使用 AOP 做空间权限？
16. `@RequireSpaceRole` 做什么？
17. `@SpaceId` 做什么？
18. `SpaceContext` 做什么？
19. 为什么不能把权限判断全部写在 Service？

### 文件

20. MySQL 和 MinIO 分别存什么？
21. 为什么使用私有 Bucket？
22. 什么是预签名 URL？
23. 为什么后端不代理大文件？
24. `inline` 和 `attachment` 有什么区别？

### Redis / 日志

25. TeamDocs 中 Redis 分别做什么？
26. 为什么使用 Lua 做登录限流？
27. 为什么使用 ZSet 做最近浏览？
28. Redis 挂了以后怎么办？
29. AOP 审计日志记录什么？
