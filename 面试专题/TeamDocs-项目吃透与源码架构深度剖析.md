...
```
JWT 本身确实是无状态的，服务端不会像传统 Session 一样保存每个 Token 的登录状态。

所以用户退出登录以后，JWT 本身不会自动失效。如果 Token 还没有过期，单纯删除前端保存的 Token 是不够的，因为如果这个 Token 已经被别人拿到了，仍然可以继续访问接口。

我的项目里是通过 Redis 做 Token 撤销来解决这个问题的。

用户退出登录时，后端会解析当前 JWT，拿到 Token 的唯一标识，然后把它加入 Redis 的撤销记录，并设置和 Token 剩余有效期一致的过期时间。

之后每次请求进入 Spring Security 的 JWT Filter 时，除了校验 JWT 的签名和过期时间，还会检查这个 Token 是否已经被 Redis 撤销。如果已经撤销，就直接返回 401，不允许继续访问。

另外，我的项目还支持修改密码后让用户之前签发的 Token 全部失效。这个场景不是简单记录某一个 Token，而是维护用户级别的失效时间水位。校验 Token 时，将 Token 的签发时间和这个水位进行比较，如果 Token 是在失效时间之前签发的，就认为它已经失效。

所以总结一下：

**JWT 负责无状态认证，Redis 负责补充“主动失效”能力。**

这样既保留了 JWT 不需要保存 Session 的特点，又能够解决退出登录、修改密码等场景下旧 Token 不能立即失效的问题。
```
