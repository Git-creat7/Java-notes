...
```
- [ ] **Q53：联合索引为什么有最左匹配原则？**
>联合索引的字段是按照从左到右的顺序组织和排序的，只有先匹配左边的字段，才能有效利用后面的字段，因此产生了**最左匹配原则**
```
...
所以查询时必须先利用最左边的字段，才能继续有效地利用后面的字段，这就是最左匹配原则

```
- [ ] **Q54：什么情况下索引会失效？**
>索引失效常见原因包括对索引字段进行函数或计算、前导模糊查询、违反联合索引最左匹配原则、隐式类型转换，以及查询数据量太大导致优化器认为全表扫描成本更低
```
...
2.秒级并发切片上传
	现代对象存储（如阿里云 OSS、AWS S3）的多段上传（Multipart Upload）天生支持幂等：同一个 UploadId 和 PartNumber 重复上传，后到达的会 直接覆盖 先到达的同名分片，不影响最终文件合并
```
- [ ] **Q67：Redis 中的操作为什么需要保证原子性？**

> Redis 保证操作原子性，主要是为了避免多个请求并发执行时操作互相穿插，导致数据状态不正确
- [ ] **Q68：Lua 脚本为什么可以保证多个 Redis 操作一起执行？**
> Redis 执行 Lua 脚本具有原子性，脚本执行期间不会被其他命令插入，因此可以保证多个 Redis 操作作为一个整体执行
...
| **`CallerRunsPolicy`（调用者运行）**     | 由**提交该任务的线程（例如主线程）直接同步执行**该任务，不交给线程池。          | **强一致性/不允许丢任务**。虽然不丢任务，但会降低提交线程的吞吐量，从而起到“降速/限流”的作用。 |
