...
1. 无哈希：完全不使用hashCode和equals，只看compare的结果
2. 强约束：Key必须实现Comparable或提供Comparator，且不能为null（因为要调用比较方法）
```Java
...
```Java
	...
	//false:右
```
4. 自动平衡：插入后通过fixAfterInsertion（变色/旋转）维持红黑树平衡
...
- 在每个方法内部加 `synchronized (this)` 锁，保证方法级别的线程安全
- 注意：遍历（iterator）时需要手动加锁（因为 iterator 是迭代器方法，包装类没给迭代过程加锁）
...
- **写法：**`List.of(),Set.of(),Map.of()`(JDK9+)
- `List<String> list = List.of("1","2");`
- **特性：** 一旦创建，长度和内容都不可变
- **注意：** `Set/Map`里的元素不能重复；`of()不允许存null`
- **价值：** 为了安全和并发性能
