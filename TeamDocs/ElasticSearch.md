...
- **别名（Alias）**：给索引起的虚拟名称，类似软链接，实现零停机切换和灵活路由
...
ES 将一个索引的数据拆分成多个**分片（Shard）**，分布存储在不同节点上。
...
**类比**：一本 1000 页的书分成 5 册，每人看一册并行翻找，比一个人翻 1000 页快。

**关键点：**

- **主分片（Primary Shard）**：创建索引时指定，**之后不可修改**（`number_of_shards`）
- **副本分片（Replica Shard）**：主分片的拷贝，可动态调整（`number_of_replicas`）
- 每个文档通过 `hash(文档ID) % 主分片数` 决定存在哪个主分片上
- 搜索时，请求被分发到所有相关分片并行执行，结果汇总返回

```
1 分片：[0] → 单节点扛所有数据

3 分片：[0] [1] [2] → 数据三等分，三节点并行
       ↓   ↓   ↓
      [R0][R1][R2] → 各自一个副本，任一节点挂了数据不丢
```

> 分片数在创建索引时就固定了，想改只能重建索引。规划时要预估数据量——太小撑不住，太多（每个分片数据太少）反而增加开销。

## 别名（Alias）

别名是给一个或多个索引起的"虚拟名称"，类似软链接。

| 操作 | 说明 |
|------|------|
| 一个别名指向一个索引 | 访问别名等同访问索引本身 |
| 一个别名指向多个索引 | 搜索时自动合并多个索引的结果 |
| 一个索引有多个别名 | 同一索引可用不同名称访问 |
| 切换别名指向 | 从旧索引切到新索引，零停机 |
...
应用代码一直访问 `prod`，重建索引后只需切换别名指向，代码无需改动。

**2. 按条件过滤（相当于视图）**

```json
PUT /products/_alias/active_products
{
  "filter": {
    "term": { "status": "active" }
  }
...
访问 `active_products` 只能看到 `status` 为 `active` 的文档。

**3. 多索引联合搜索**

别名指向 `logs-2026-05` + `logs-2026-04`，一次查询跨两个索引。
...
> **一句话：别名解耦应用代码和实际索引名，实现无缝切换和灵活路由。**
