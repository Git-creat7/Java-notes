...
| **`map`**      |   类型转换   | 把一种对象换成另一种（如把 `Student` 转成 `String` 姓名）`list.stream().map(s -> Integer.parseInt(s.split("-")[1]))` |
...
| **`forEach`**  | 遍历        | 打印或消费数据                            |     |
| **`count`**    | 统计个数      | 返回 `long`                          |     |
| **`collect`**  | **收集**转集合 | **最重要！** 把流转回 `List`, `Set`, `Map` |     |
| **`toArray`**  | **收集**转数组 | 收集流中的数据，放到数组中                      |     |
